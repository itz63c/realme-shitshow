...
public final class Util {
    ...
    public static void checkOffsetAndCount(long j, long j2, long j3) {
        if ((j2 | j3) < 0 || j2 > j || j - j2 < j3) {
            throw new NetonException((Throwable) new ArrayIndexOutOfBoundsException());
        }
    }
    ...
    public static int checkDuration(String str, long j, TimeUnit timeUnit) {
        if (j < 0) {
            throw new NetonException((Throwable) new IllegalArgumentException(str + " < 0"));
        } else if (timeUnit == null) {
            throw new NetonException((Throwable) new NullPointerException("unit == null"));
        } else {
            long millis = timeUnit.toMillis(j);
            if (millis > 2147483647L) {
                throw new NetonException((Throwable) new IllegalArgumentException(str + " too large."));
            } else if (millis != 0 || j <= 0) {
                return (int) millis;
            } else {
                throw new NetonException((Throwable) new IllegalArgumentException(str + " too small."));
            }
        }
    }
}
