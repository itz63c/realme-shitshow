...
public final class RealInterceptorChain implements Interceptor.Chain {
    ...
    public final Response proceed(Request request2, StreamAllocation streamAllocation2, HttpCodec httpCodec2, RealConnection realConnection) {
        ...
        if (this.httpCodec != null && !this.connection.supportsUrl(request2.url())) {
            throw new NetonException((Throwable) new IllegalStateException("network interceptor " + this.interceptors.get(this.index - 1) + " must retain the same host and port"));
            ...
            if (httpCodec2 != null && this.index + 1 < this.interceptors.size() && realInterceptorChain.calls != 1) {
                throw new NetonException((Throwable) new IllegalStateException("network interceptor " + interceptor + " must call proceed() exactly once"));
            } else if (intercept == null) {
                throw new NetonException((Throwable) new NullPointerException("interceptor " + interceptor + " returned null"));
            } else if (intercept.body() != null) {
                return intercept;
            } else {
                throw new NetonException((Throwable) new IllegalStateException("interceptor " + interceptor + " returned a response with no body"));
            }
        } else {
            throw new NetonException((Throwable) new IllegalStateException("network interceptor " + this.interceptors.get(this.index - 1) + " must call proceed() exactly once"));
        }
    }
}
